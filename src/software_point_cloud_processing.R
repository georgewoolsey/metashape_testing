    ...
    dplyr::mutate(
      ...
      , file_name = ###### !!!!! MAKE ADAPTABLE TO DIFFERENT FILE NAME STRUCTURES
        dplyr::case_when(
          ...
          , file_full_path %>% 
            tolower() %>% 
            stringr::str_detect("_group(.*?)_densified_point_cloud") ~ file_full_path %>% 
              toupper() %>% 
              stringr::word(-1, sep = fixed(paste0(study_site,"/"))) %>% 
              stringr::str_replace_all("/", "_") %>% 
              stringr::word(1, sep = fixed(".")) %>% 
              stringr::str_remove_all("_GROUP(.*?)_DENSIFIED_POINT_CLOUD")
...
regional_dbh_est_fn = function(my_study_site){
  ...
  if(file.exists(paste0(outdir_site, "/regional_dbh_height_model_predictions.csv"))==F){
    ...
    ### export tabular
      write.csv(
          treemap_trees_df
          , paste0(outdir_site, "/regional_dbh_height_model_training_data.csv")
          , row.names = F
        )
    ...
    mod_nl_pop = brms::brm(
      ...
      , file = paste0(outdir_site, "/regional_dbh_height_model")
      , file_refit = "never"
    )
    # plot(mod_nl_pop)
    # summary(mod_nl_pop)
...
process_raw_las_fn = function(my_las_file_path){
    ...
    #################################################################################
      # create spatial index files (.lax)
        flist_temp = create_lax_for_tiles(my_las_file_path)
      ...
      # save processing attributes
        sf::st_write(
          process_data
          , paste0(delivery_dir, "/", f_name, "_", "process_data_las_tiling.gpkg")
          , quiet = TRUE, append = FALSE
        )
      # clean up
        remove(list = ls()[grep("_temp",ls())])
    ...
    #################################################################################
      ...
      ###################
        lasr_dtm_norm_fn = function(
          dtm_file_name = paste0(delivery_dir, "/", f_name, "_", "dtm_", desired_dtm_res, "m.tif")
          , frac_for_tri = 1
      ...
      ######################################
        ...
        # check for errors other than too few points for triangulation which happens on edge chunks with few points that have noise
        # check for errors other than too few points for triangulation which happens on edge chunks with few points that have noise
          error_list_temp = 
            lasr_ans_list %>% 
            purrr::transpose() %>% 
          ...
          if(nrow(dbh_training_data_temp)>10){
            ...
            if(tolower(local_dbh_model) == "rf"){
              ...
              stem_prediction_model = brms::brm(
                ...
                , iter = 4000, warmup = 2000, chains = 4
                , cores = lasR::half_cores()
                , file = paste0(delivery_dir, "/", f_name, "_", "local_dbh_height_model")
                , file_refit = "always"
              )
            }
            ...
            purrr::map(function(x){
              ...
              sf::st_write(
                crowns_sf_with_dbh %>% 
                  dplyr::filter(grp == x) %>% 
                  dplyr::select(-c(grp))
                , paste0(delivery_dir, "/", f_name, "_", "final_detected_crowns_",x,".gpkg")
                , append = FALSE
                , quiet = TRUE
              )
              ...
              sf::st_write(
                ...
                , paste0(delivery_dir, "/", f_name, "_", "final_detected_tree_tops_",x,".gpkg")
                , append = FALSE
                , quiet = TRUE
              )
              return(
                dplyr::tibble(
                  crowns_file = paste0(delivery_dir, "/", f_name, "_", "final_detected_crowns_",x,".gpkg")
                  , trees_file = paste0(delivery_dir, "/", f_name, "_", "final_detected_tree_tops_",x,".gpkg")
                )
              )
            }) %>% 
            dplyr::bind_rows()
...
######################################################################################################
######################################################################################################
# CALL FUNCTION TO PROCESS EACH ALTERNATE LAZ FILE
######################################################################################################
######################################################################################################
######################################################################################################
  ...
  processed_tracking_data = las_list_df %>%
    ...
    #     | (study_site== "KAIBAB_LOW" & file_name == "ORIGINAL_ORIGINAL_HIGH")
    #     | (study_site== "N1" & file_name == "ORIGINAL_ORIGINAL_HIGH")        
    #     # | (study_site== "SQ09_02" & file_name == "ORIGINAL_ORIGINAL_HIGH")   
    #     # | (study_site== "WA85_02" & file_name == "HALF_ORIGINAL_HIGH")   
    #     # | (study_site== "WA85_02" & file_name == "ORIGINAL_ORIGINAL_HIGH")   
    #     # | (study_site== "WA85_02" & file_name == "QUARTER_ORIGINAL_HIGH")  
    # ) %>%
    dplyr::pull(file_full_path) %>% 
...
# # pix4d
# # 1 | (study_site== KAIBAB_HIGH & file_name == DOUBLE_ORIGINAL_HIGH)
# # 2 | (study_site== KAIBAB_HIGH & file_name == QUARTER_ORIGINAL_HIGH)
# # 3 | (study_site== KAIBAB_LOW & file_name == HALF_ORIGINAL_HIGH)
# # 4 | (study_site== KAIBAB_LOW & file_name == ORIGINAL_ORIGINAL_HIGH)
# # 5 | (study_site== N1 & file_name == ORIGINAL_ORIGINAL_HIGH)
# # 6 | (study_site== SQ09_02 & file_name == ORIGINAL_ORIGINAL_HIGH)
# # 7 | (study_site== WA85_02 & file_name == ORIGINAL_ORIGINAL_HIGH)
# # 8 | (study_site== WA85_02 & file_name == QUARTER_ORIGINAL_HIGH)