...
priors_temp %>% 
  ...
  dplyr::mutate(
    distrib = paste0(
      ...
      , .dist
      , "("
      , a %>% tidyr::extract_numeric() %>% round(2)
      , ","
      , b %>% tidyr::extract_numeric() %>% round(2)
      , ")"
    )
  ) %>% 
  ggplot(., aes(dist = .dist, args = .args)) +
  facet_wrap(vars(distrib), scales = "free") +
  ggdist::stat_halfeye(
    aes(fill = prior),
    n = 10e2,
    show.legend = F
  ...
  scale_fill_manual(
    values = viridis::turbo(n=15, begin = 0.1, end = 0.9) %>% sample()
  ) +
  theme_bw() +
  theme(
...
############################
  ...
  scale_x_continuous(
    limits = c(-9.0,9.0)
    , breaks = seq(-8,8,4)
    , labels = seq(-8,8,4) %>% scales::number(accuracy = 1)
  ) &
  theme(
    axis.title.y = element_blank()
    , plot.subtitle = element_text(face = "bold", hjust = 0.0)
...
dplyr::bind_rows(
  ...
  labs(
    x = "ESS"
    , subtitle = "MCMC chain convergence check for effective sample size (ESS) values"
    , y = ""
  ) +
  theme_light() +
  theme(
...
# plot together with patchwork
# height mean error
brms_ht_me_mod %>% plt_ess_temp() + labs(title = "brms_ht_me_mod") +
# height rmse
brms_ht_rmse_mod %>% plt_ess_temp() + labs(title = "brms_ht_rmse_mod") +
# dbh me
brms_dbh_me_mod %>% plt_ess_temp() + labs(title = "brms_dbh_me_mod") +
# dbh rmse
brms_dbh_rmse_mod %>% plt_ess_temp() + labs(title = "brms_dbh_rmse_mod") &
  theme(plot.subtitle = element_blank())
```
