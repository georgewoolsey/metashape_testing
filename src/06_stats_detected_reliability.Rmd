...
```{r}
# sort it
fltr_sftwr_draws_temp = fltr_sftwr_draws_temp %>%
  tidybayes::median_hdi(value) %>%
  dplyr::inner_join(
    ptcld_validation_data %>% dplyr::distinct(software, depth_maps_generation_quality, depth_maps_generation_filtering_mode)
    , by = dplyr::join_by(software, depth_maps_generation_quality, depth_maps_generation_filtering_mode)
...
# table it
fltr_sftwr_draws_temp %>%
  dplyr::select(-c(.point,.interval, .width,.row,software)) %>% 
  kableExtra::kbl(
    digits = 2
    , caption = "Height Mean Error (m)<br>95% HDI of the posterior distribution of conditional mean group estimates"
    , col.names = c(
  ...
  kableExtra::kable_styling() %>% 
  kableExtra::pack_rows(index = table(forcats::fct_inorder(fltr_sftwr_draws_temp$software))) %>% 
  kableExtra::scroll_box(height = "8in")
```
...
draws_temp %>% 
  ...
  kableExtra::kbl(
    digits = 2
    , caption = "DBH RMSE (m)<br>95% HDI of the posterior distribution of conditional mean group estimates"
    , col.names = c(
      "quality", "filtering"
      , "median"
      , "conf.low", "conf.high"
