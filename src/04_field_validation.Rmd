...
```{r, include=FALSE, eval=FALSE}
ptcld_validation_data %>% 
  dplyr::select(f_score, basal_area_pct_error, tree_height_m_me, tree_height_m_rmse, dbh_cm_me, dbh_cm_rmse) %>% 
  dplyr::ungroup() %>% 
  dplyr::summarise(
    dplyr::across(
  ...
  dplyr::mutate(
    value = dplyr::case_when(
      metric == "basal_area_pct_error" ~ scales::percent(value, accuracy = 0.1)
