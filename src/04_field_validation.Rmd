...
ptcld_processing_data = ptcld_processing_data %>% 
  dplyr::slice_sample(n = 15, replace = F)
```
...
# will pass list of unique processing_id for each site
...
#####################################################
  ...
  validate_file_fn = function(p_id, fld_dta, plt_dta){
    ...
    # write tree list 
      tl_fnm = paste0(
          field_uas_comparison$processed_data_dir[1]
          , "/"
          , field_uas_comparison$file_name[1]
    ...
    # aggregate statistics for return
      return_dta =
        ptcld_processing_data %>% 
        dplyr::filter(processing_id == p_id) %>% 
        # attach f score
        ...
        dplyr::mutate(validation_file_full_path = tl_fnm)
...
field_uas_comparison_temp %>%
  ...
  ggplot(mapping = aes(x = dbh_diff_pct, y = plot_lab)) + # dbh_diff_pct
    ...
    scale_x_continuous(
      ...
      , limits = c(
        -2,2
        # -max(field_uas_comparison_temp$abs_dbh_diff_pct, na.rm = T)
        # , max(field_uas_comparison_temp$abs_dbh_diff_pct, na.rm = T)
      )
    ) +
    labs(
