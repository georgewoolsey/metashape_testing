  ...
  print(
    ggplot(
      data = ptcld_validation_data %>% 
        dplyr::group_by(dplyr::pick(my_var)) %>%
...
# sum_stats_dta("depth_maps_generation_quality")
```
...
contrast_temp = contrast_temp %>% 
  ...
  dplyr::mutate(
    ...
    , depth_maps_generation_filtering_mode = depth_maps_generation_filtering_mode %>% 
      factor(
        ordered = T
        , levels = levels(ptcld_processing_data$depth_maps_generation_filtering_mode)
      )
  ) %>% 
...
```{r}
ptcld_processing_data$timer_total_time_mins %>% 
  summary()
```
