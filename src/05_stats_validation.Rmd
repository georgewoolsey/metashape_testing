...
brms_contrast_temp %>% 
  dplyr::ungroup() %>% 
  dplyr::count(contrast, median_hdi_est, pct_gt_zero_lab,pct_gt_zero_lab_sm)
```
...
brms_f_mod5_priors_temp %>% 
  ...
  scale_fill_manual(
    values = viridis::turbo(n=12, begin = 0.1, end = 0.9) %>% sample()
  ) +
  theme_bw() +
  theme(
...
Averaging across all levels of filtering mode, dense cloud quality setting, study site, and software the process does a better job detecting overstory trees than understory trees. Because this is a Bayesian analysis we can quantify this probability: there is a `r brms_contrast_temp$pct_gt_zero[1] %>% scales::percent(accuracy = 0.1)` that overstory trees will be detected with better accuracy than understory trees (see coloring on posterior predictive distribution above)
