...
ggplot2::ggsave("../data/qlty_sftwr_comp_mod4.png", height = 6, width = 8)
```
...
ggplot2::ggsave("../data/qlty_sftwr_comp_mod5.png", height = 6, width = 8)
```
...
ggplot2::ggsave("../data/sftwr_qlty_comp_mod5.png", height = 8, width = 6)
```
...
First, we'll look at the main effect of overstory/understory on F-score by collapsing across the filtering mode, depth map quality setting, study site, and software and then performing a pairwise comparison between overstory and understory
...
ou_ptcld_validation_data %>%
  ...
  ggplot(
    ...
    scale_fill_viridis_d(option = "magma", begin = 0.3, end = 0.9, drop = F) +
    scale_x_continuous(breaks = scales::extended_breaks(n=8)) +
    labs(
      y = "", x = "f-score"
      , subtitle = "posterior distribution of conditional means with 95% HDI"
...
brms_contrast_temp %>% 
  ...
  scale_x_continuous(breaks = scales::extended_breaks(n=8)) +
  facet_wrap(facets = vars(story), ncol = 2) +
  labs(
    y = "software"
    , x = "constrast (f-score)"
...
ggplot2::ggsave("../data/sftwr_stry_comp_mod6.png", height = 8, width = 6)
```
